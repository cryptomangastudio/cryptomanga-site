...
> **🔒 事業クローズ(2026-07-21)**: ポートフォリオ見直しにより本事業は終了(創業者決定)。
> 以後、開発・運用への時間投資は行わない。コードと検証設計(ウォークフォワード+DSR、
> ペーパー→実弾の段階昇格ゲート)は資産として保存する。実弾投入(`CRYPTOBOT_LIVE`)は行わないこと。
> 経緯 → `docs/2026-07_事業ポートフォリオと優先順位.md`(リポジトリルート)
