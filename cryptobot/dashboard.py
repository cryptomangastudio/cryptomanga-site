...
from bot.strategy import moving_avg
...
def _ma_series(closes: list[float], window: int, ma_type: str = "sma") -> list[float | None]:
    """終値列に対する移動平均の系列(足りない区間はNone)。戦略の計算と一致させる。"""
    out: list[float | None] = []
    for i in range(len(closes)):
        out.append(moving_avg(closes[: i + 1], window, ma_type) if i + 1 >= window else None)
    return out
...
class Dashboard:
    ...
    def strategy_chart(self, sym: str) -> dict | None:
        ...
        mt = self.cfg.ma_cross.ma_type
        fast = _ma_series(c, fast_n, mt)
        slow = _ma_series(c, slow_n, mt)
        gap = None
