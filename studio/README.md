...
- [x] ベンチマーク徹底洗い出し＋勝てる筋のピック（→ `docs/strategy/03-benchmarks.md` / `04-winnable-plays.md`）
- [ ] **1体目キャラの企画ドラフト**（← 次のアクション。旗艦候補＝Pick 1「社会あるある擬人化」）
- [ ] 90日SNS検証（成功/失敗ラインは `docs/playbooks/launch-90day.md`）

## 勝ちの型（1行）
かわいい×内向きの毒 → AIで多産→SNSで多打席→当たりだけ製品化 / 入口ほぼ0円・増幅はライセンスアウト / AI開示・全年齢・完全オリジナル。詳細は `docs/strategy/04-winnable-plays.md`。
